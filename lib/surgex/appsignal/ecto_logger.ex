if Code.ensure_loaded?(Appsignal) do
  defmodule Surgex.Appsignal.EctoLogger do
    ...
    - `:query_stages` - allows to alter the default duration calc behavior in which all three Ecto
      ...
      configure multiple events for specific stages.
    ...
    Additional log for queue time without query and decode times:
    ...
    defp get_event_name(event_name_parts, telemetry_event) when is_list(event_name_parts) do
      event_name_parts
      |> Enum.each(&resolve_event_name_part(&1, telemetry_event))
      |> Enum.reverse()
      |> Enum.join(".")
    end
    ...
    def log(entry, event_name, query_stages) do
      ...
      case TransactionRegistry.lookup(self()) do
        ...
        %Transaction{} = transaction ->
          ...
          Transaction.record_event(transaction, event_name, "", entry.query, duration, 1)
      end
